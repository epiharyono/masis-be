...
class GaleryController extends Controller
{
   static function HomeData($req){
        ...
        $query  = DB::table('ta_galery')->where('status',1)->orderby('created_at','desc');
        if($req->search){
    ...
    static function GetDataXXX($req){
         $success = true; $message = 'Sukses Get Data';

         $query  = DB::table('ta_galery')->where('status',1);
         if($req->search){
     ...
     static function GetRincXXX($slug){
