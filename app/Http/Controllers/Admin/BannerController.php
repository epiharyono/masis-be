...
class BannerController extends Controller
{
   ...
   static function GetData($req){
        ...
        $super   = 1;
        $user    = Auth::user();
        $ha = HALocal::HakAksesUser($user->nip,3);
        $success = true; $message = 'Sukses Get Data Users';
        $query  = DB::table('ta_banner')->orderby('created_at','desc');
        if($req->search){
    ...
    static function UpdateBySlug($slug,$req){
         ...
         try {
             if ($req->hasFile('image')) {
               ...
               $file->storeAs($folder, str_replace(' ', '_', $file_name),'my_files');
               $image = $folder.'/'.$file_name;
               DB::table('ta_banner')->where('slug',$req->slug)->update([
                  'judul' => $req->judul,
                  'image'  => $image,
                  'keterangan'  => $req->keterangan,
                  'status'  => $req->status,
